...
class Amazon_books(object):
    def scrap(self):
        ...
        for contain in container:
            try:
                self.title=contain.find_all("a",{"class":"a-link-normal"})#get the title of books
                #print(title[1].text.strip())
            except:
                pass
            self.author=contain.find_all("span",{"class":"a-size-small a-color-base"})#get authors
            #print(author[0].text)
            self.price=contain.find_all("span",{"class":"a-size-base a-color-price"})#get price
            #print(price[0].text)
            try:
                self.star=contain.find_all("i",{"class":"a-icon a-icon-star a-star-4-5"})#get star
                #print(star[0].text)
            except:
                pass
            try:
